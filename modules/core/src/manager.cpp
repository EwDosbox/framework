...
void ApplicationManager::init(int width = 1280, int height = 720, std::string title = "Application", int major = 4, int minor = 5)
{
	...
	// glfwWindowHint(GLFW_TRANSPARENT_FRAMEBUFFER, 1); // we leave it for manual override in PB112, as it is messing up with PV227 lectures where we need 0 in alpha.
...
void ApplicationManager::run(IApplication& application)
{
	...
	// Forces the resize of the application to the width and height of the framebuffer.
	int fb_width, fb_height;
	glfwGetFramebufferSize(window, &fb_width, &fb_height);
	application.on_resize(fb_width, fb_height);
...
bool ApplicationManager::is_fail() const
{
	return fail;
}

void ApplicationManager::disable_resize() const
{
	glfwSetWindowAttrib(window, GLFW_RESIZABLE, GLFW_FALSE);
}

void ApplicationManager::set_multisampling_per_pixel(int samples)
{
...
void GLAPIENTRY ApplicationManager::opengl_message_callback(GLenum source, GLenum type, GLuint id, GLenum severity, GLsizei length, const GLchar* message, const void* userParam)
{
